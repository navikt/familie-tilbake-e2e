...
class Saksbehandler(
    ...
    fun erBehandlingAvsluttet(behandlingId: String, resultat: Behandlingsresultatstype){
        Vent.til( { familieTilbakeKlient.hentBehandling(behandlingId)?.status == Behandlingsstatus.AVSLUTTET },
        ...
        if (henlagttyper.any() {it == resultat}){
            ...
            assertTrue(behandling.behandlingsstegsinfo.filter {
                it.behandlingssteg != Behandlingssteg.VARSEL }.all {
            ...
            assertTrue(behandling.resultatstype == Behandlingsresultatstype.HENLAGT, "Forventet resultat: HENLAGT, Behandlingens resultat: ${behandling.resultatstype}")
        }
        else if (iverksatttyper.any {it == resultat}){
            ...
            assertTrue(behandling.resultatstype == resultat, "Forventet resultat: $resultat, Behandlingens resultat: ${behandling.resultatstype}")
        }
        else { throw Exception("Behandling var i status AVSLUTTET men resultattypen angitt var ${behandling!!.resultatstype} som ikke er et gyldig resultat for en avsluttet behandling") }
