    ...
    private fun til(supplier: Supplier<Boolean?>, timeoutInSeconds: Int, errorMessageProducer: Supplier<String?>) {
        ...
        while (!supplier.get()!!) {
            if (LocalDateTime.now().isAfter(end)) {
                throw RuntimeException(String.format("Async venting timet ut etter %s sekunder fordi: %s",
                    timeoutInSeconds, errorMessageProducer.get()))
            }
            try {
                Thread.sleep(500)
            } catch (e: InterruptedException) {
                throw RuntimeException(String.format("Async venting interrupted ut etter %s sekunder fordi: %s",
